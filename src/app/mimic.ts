...
export class Mimic extends Object3D {
    ...
    constructor(anschors: Vector3[]) {
        ...
        anschors.forEach(a => {
            ...
            for (var i = 0; i < Math.random()*5+2; i++) {
                this.anchors.push(a.clone().add(new Vector3(Math.random() * rr - rr/2, Math.random() * rr - rr/2, 0)))
            }
    ...
    update(position: Vector3): void {
        ...
        this.boxmesh.position.z=0.02*Math.sin(time)
        ...
        this.anchors.forEach(a => {
            ...
            if (d < (mdd - md)) {
                if (!this.ms.has(a)) {
                    const geometry = new THREE.BufferGeometry().setFromPoints([new Vector3(0, 0, 0)]);
                ...
                if (e) {
                    ...
                    const minr = 0.9 * (mdd - md)
                    const maxr = 1 * (mdd - md)
                    const dd = Math.max(0, points3.length / (maxr - minr) * (d - minr));

                    e.setDrawRange(dd, points3.length)
                }
